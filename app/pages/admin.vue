            ...
            <div
                ...
                <div v-if="!isOpenPreview" class="relative min-h-[500px]
                    ...
                    <div v-if="!propertyStore.adminListLoaded || isListLoading"
                        ...
                        <UIcon name="i-heroicons-arrow-path" class="w-12 h-12 animate-spin text-cbre_primary_1" />
...
const { isLoading, lastAction } = storeToRefs(statusStore);
...
const isListLoading = computed(() => isLoading.value && lastAction.value === 'fetchAdminList');
...
onMounted(async () => {
    if (!fullAdminList.value.length) {
        await propertyStore.fetchAdminList();
    }
});

// Watchers
watch(() => statusStore.lastAction, (newAction) => {
    // 예: 삭제 후 리스트 갱신 완료 시 로딩 상태 해제 등
    if (!isLoading.value && newAction === 'deleteProperty') {
        // ...
    }
});
