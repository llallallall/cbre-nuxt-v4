                        ...
                        <div class="flex justify-between items-center mb-3">
                            <h3 class="text-xl font-semibold text-cbre_primary_3">
                                ...
                                <span class="text-sm font-normal text-gray-500 ml-2">({{
                                    activeSheet.rows.length }} rows)</span>
                            </h3>
                            <div v-if="activeSheet.errors.length > 0" class="text-red-600 text-sm font-medium">
                                {{ activeSheet.errors.length }} Issues Found
                            </div>
                        </div>

                        <!-- Error List for Active Sheet -->
                        <div v-if="activeSheet.errors.length > 0"
                            class="mb-4 bg-red-50 border border-red-200 rounded-lg p-3 max-h-40 overflow-y-auto">
                            <ul class="list-disc list-inside text-sm text-red-700">
                                <li v-for="(err, i) in activeSheet.errors" :key="i">{{ err }}</li>
                            </ul>
                        </div>

                        <!-- UTable Replacement -->
                        <UTable :columns="activeSheet.columns as any" :rows="activeSheet.rows"
...
const activeSheet = computed(() => sheetsData.value[activeSheetIndex.value]);
...
const handleFileUpload = async () => {
    ...
    if (hasCriticalErrors.value) {
        showToast("Please fix validation errors before uploading.", 'warning');
        return;
    }
    ...
    try {
        ...
        if (data.value) {
            ...
            showToast('File uploaded successfully! All properties have been imported.', 'success', { timeout: 5000 });
            ...
            showToast(`File upload failed. ${error.value?.statusMessage || 'Please check the console for details.'}`, 'danger', { timeout: 5000 });
        }

    } catch (error) {
        console.log(error);
        showToast('An error occurred during upload. Please check the console.', 'danger', { timeout: 5000 });
