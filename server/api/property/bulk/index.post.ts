    ...
    try {
        await db.transaction(async (tx) => {
            ...
            for (const row of floors) {
                ...
                if (propId) {
                    await tx.insert(schema.floor).values({
                        ...
                        totalAreaSqm: row['Total Area (sqm)'] ? parseFloat(row['Total Area (sqm)']) : null,
                        // Map other fields
                    });
                }
            }
            ...
            for (const row of floorPartials) {
                ...
                if (propId) {
                    // Find parent floor
                    const floor = await tx.query.floor.findFirst({
                        where: and(
                            eq(schema.floor.propertyId, propId),
                            eq(schema.floor.type, row['Type']),
                            eq(schema.floor.floor, parseInt(row['Floor']))
                        )
                    });
            ...
            for (const row of transactions) {
                ...
                if (propId && row['Ref_ID']) {
                    const [transaction] = await tx.insert(schema.transaction).values({
                        ...
                        executionDate: row['Execution Date'] ? new Date(row['Execution Date']).toISOString() : new Date().toISOString(),
                    }).returning();
