...
## **0. 🏗️ CSS Architecture & Efficiency (CSS 설계 원칙)**

효율적인 스타일링과 일관된 유지보수를 위해 다음 3단계 원칙을 따릅니다.

### **Level 1: Tailwind CSS (Primary - 최우선)**

*   **용도:** 여백(`p-4`), 배치(`flex`, `grid`), 간단한 색상(`text-cbre-green`), 반응형 등 **일회성 스타일**.
*   **원칙:** 가능한 모든 스타일은 Tailwind 유틸리티 클래스로 해결합니다. 별도 CSS 파일을 만들지 않아 가장 효율적입니다.
*   **예시:** `<div class="flex items-center justify-between p-4 bg-white">`

### **Level 2: Global CSS (Secondary - 재사용)**

*   **위치:** `app/assets/css/main.css`
*   **용도:**
    *   **3곳 이상 반복**되는 UI 패턴 (예: `.cbre-button-primary`, `.cbre-nav-item`).
    *   디자인 시스템의 핵심 스타일 (폰트, 리셋, 공통 레이아웃).
    *   복잡한 Tailwind 클래스 나열을 의미 있는 이름으로 추상화할 때.
*   **원칙:** `@apply`를 사용하여 Tailwind 설정을 그대로 활용합니다.
*   **예시:**
    ```css
    /* main.css */
    .cbre-button-primary {
        @apply bg-cbre-green text-white px-6 py-3 hover:bg-cbre-accent transition-colors;
    }
    ```

### **Level 3: Scoped CSS (Tertiary - 격리)**

*   **위치:** 컴포넌트 내 `<style scoped>`
*   **용도:**
    *   해당 컴포넌트에서만 쓰이는 **매우 특수한 애니메이션**이나 **동적 로직**.
    *   외부 라이브러리(Swiper 등)의 스타일 오버라이딩.
*   **원칙:** 사용을 최소화하여 스타일 파편화를 방지합니다.
