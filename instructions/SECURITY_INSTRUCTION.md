...
### ⚠️ 중요: Nuxt 설정 파일 문법 준수
`nuxt-security`와 같은 모듈은 `nuxt.config.ts`의 문법 오류나 잘못된 중첩(nesting)에 매우 민감합니다. 설정 파일이 손상되면 500 Server Error (`html.replace is not a function` 등)가 발생할 수 있으므로, 항상 올바른 JSON/JS 객체 구조를 유지해야 합니다.

### 🔧 Mapbox 및 Google API를 위한 CSP 설정 예시
`nuxt-security` 사용 시 외부 스크립트(Mapbox, Google Maps) 로드를 위해 `contentSecurityPolicy`를 적절히 완화해야 합니다.

```typescript
contentSecurityPolicy: {
  "img-src": ["'self'", "data:", "blob:", "https://*.mapbox.com", "https://*.googleapis.com", "https://*.gstatic.com"],
  "script-src": ["'self'", "'unsafe-eval'", "'unsafe-inline'", "blob:", "https://*.mapbox.com", "https://*.googleapis.com"],
  "connect-src": ["'self'", "blob:", "https://*.mapbox.com", "https://events.mapbox.com", "https://*.googleapis.com"],
